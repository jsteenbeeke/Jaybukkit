...
public class TransferHandler extends PlayerAwareCommandHandler {
	...
	public CommandMatcher getMatcher() {
		return ifNameIs("transfer").itMatches();
	}

	@Override
	public ParameterIntegrityChecker getParameterChecker() {
		return ifArgCountIs(2).andArgumentLike(0, DECIMAL)
	...
	public void onAuthorizedAndPlayerFound(Player player, Command command,
		...
		OfflinePlayer targetPlayer = plugin.getServer().getOfflinePlayer(
				args[1]);
		...
		if (targetPlayer != null) {
			...
			if (balance.compareTo(amount) >= 0) {
				...
				Messages.send(
						player,
						String.format("&aTransfered &e%s&a to &e%s",
